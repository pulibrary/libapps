(function ($) {
  ...
  $(document).ready(function () {
    ...
    $('#allsearch-block-form').submit(function () {
      if ($(this).find("input:first").val() == "" || $(this).find("input:first").val() == all_search_hint) {
        if($('#allsearch-error').length == 0) {
          $("<div id='messages' class='grid-12'><div id='allsearch-error' class='messages error'><h2 class='element-invisible'>Error message</h2>"+all_search_error+"</div></div>").insertBefore('.l-content');
        }
